...
const ITEM_H = 38;
/** Roughly six rows -- past that the panel starts covering the screen it filters. */
...
interface PixelSelectProps {
    ...
    /** Share of the row this control takes, against its siblings. */
    flex: number;
}
 ...
 * these controls sit inside a screen's vertical ScrollView, and Android clips a
 * child that overflows its scrolling parent -- the list would have been cut off
 * at the first row. The trigger is measured in window coordinates on press and
 * the panel is placed at those coordinates, so it still reads as having dropped
 * out of the button.
 *
 * Lives here rather than inside FilterRow because the Goals form picks its start
 * date and its times-per-week with the same control; the alternative was a second
 * copy of the measure-and-place logic below.
 */
export default function PixelSelect({ eyebrow, options, value, onChange, flex }: PixelSelectProps) {
    ...
    // what happens to a control near the bottom of a short screen.
